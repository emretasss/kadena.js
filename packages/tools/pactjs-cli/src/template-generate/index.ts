...
const TemplateGenerateOptions = z.object({
  clean: z
    .boolean({
      ...
      invalid_type_error: 'Error: -c, --clean is optional but can only be a flag',
    })
    .optional(),
  file: z.string({
    /* eslint-disable-next-line @typescript-eslint/naming-convention */
...
export function templateGenerateCommand(program: Command, version: string): void {
  program
    ...
    .option('-f, --file <file-or-directory>', 'File or directory to use to generate the client')
