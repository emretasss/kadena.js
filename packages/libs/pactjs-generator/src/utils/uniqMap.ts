export const uniqMap = <TIn = unknown, TOut = unknown, TId = unknown>(
  ...
  identifierFn: (element: TIn, index: number, array: TIn[]) => TId = (el) => el as unknown as TId,
): TOut[] => {
  const iteraterFn = (array: TIn[]): TOut[] => {
    const elements: TId[] = [];
    const mappedElements: TOut[] = [];
    const createIterator = (idFn: typeof identifierFn): ((element: TIn, index: number, array: TIn[]) => void) => {
