...
Define keyset as NAME with KEYSET, or if unspecified, read NAME from message payload as keyset, similarly to
'read-keyset'. If keyset NAME already exists, keyset will be enforced before updating to new value.
...
Read KEY from message data body as keyset (\{ "keys": KEYLIST, "pred": PREDFUN \}). PREDFUN should resolve to a keys
predicate.
