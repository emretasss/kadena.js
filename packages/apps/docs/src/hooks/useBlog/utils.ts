...
export const getInitBlogPosts = (menuData: IMenuData[], offset: number, limit: number): IMenuData[] => {
  const STARTBRANCH = '/docs/blogchain';

  const startBranch = menuData.find((item) => item.root === STARTBRANCH) as IMenuData;
