...
describe('landing page layout', () => {
  ...
  describe('desktop', () => {
    ...
    it('shows the sidemenu', () => {
      menu().should('exist');
      menu().get('[data-cy="sidemenu-submenu"]').find('ul:first').children().should('have.length', 4);
    });
    ...
    it('shows the title header', () => {
      ...
      header().find('h5').contains('The safest, most user-friendly language for smart contracts');
